...
impl<T> WindowState<T> {
    ...
    fn remove_shell(&mut self, id: id::Id) -> Option<()> {
        // Include AllScreens / TargetScreen bars (`becreated == false`), not only
        // dynamically created popups. Otherwise monitor unplug leaves dead units.
...
impl<T> WindowState<T> {
    ...
    /// Set iced (or other) binding data on a unit once its window id is known.
    pub fn set_unit_binding(&mut self, id: id::Id, binding: T) {
        if let Some(unit) = self.get_mut_unit_with_id(id) {
            unit.set_binding(binding);
        }
    }
...
impl<T: 'static> OutputHandler for WindowState<T> {
    ...
    fn output_destroyed(
        ...
        // Request close while units are still in `self.units` so iced can resolve
        // bindings / aliases and destroy surfaces via the normal close path.
        // (Previously we extract_if'd units first, which broke iced cleanup.)
        for unit in self.units.iter_mut() {
            let should_close = !unit.wl_surface.is_alive()
                || unit
                    .wl_output
                    .as_ref()
...
impl<T> Dispatch<zxdg_output_v1::ZxdgOutputV1, ()> for WindowState<T> {
    fn event(
        ...
        let Some(index) = state.units.iter().position(|info| {
            ...
            // TargetScreen: output came back and name is now known — recreate bar.
            if let (StartMode::TargetScreen(target), zxdg_output_v1::Event::Name { name }) =
                (&state.start_mode, &event)
            {
                if name == target {
                        ...
                        .find(|(_, info)| info.zxdgoutput == *proxy)
                    {
                        let already = state
                            .units
                            .iter()
                            .any(|u| u.wl_output.as_ref() == Some(output));
                        if !already {
                            state
                                .message
                                .push((None, DispatchMessageInner::NewDisplay(output.clone())));
                        }
                    }
                }
            }
...
impl<T: 'static> WindowState<T> {
            ...
            + 'static,
    {
        ...
        let process_window_state = |window_state: &mut WindowState<T>, event_handler: &mut F| {
                ...
                match msg {
                    ...
                    (_, DispatchMessageInner::NewDisplay(output_display)) => {
                        let cached = window_state
                            .xdg_info_cache
                            .iter()
                            .find(|(o, _)| o == output_display)
                            .map(|(_, info)| info.clone());
                        let zxdg_info = if let Some(info) = cached {
                            info
                        } else {
                            let zxdgoutput =
                                xdg_output_manager.get_xdg_output(output_display, &qh, ());
                            let info = ZxdgOutputInfo::new(zxdgoutput);
                            window_state
                                .xdg_info_cache
                                .push((output_display.clone(), info.clone()));
                            info
                        };

                        let already_bound = window_state
                            .units
                            .iter()
                            .any(|u| u.wl_output.as_ref() == Some(output_display));
                        if already_bound {
                            continue;
                        }

                        let should_create = match &window_state.start_mode {
                            StartMode::AllScreens => true,
                            StartMode::TargetScreen(name) => {
                                !zxdg_info.name.is_empty() && zxdg_info.name == *name
                            }
                            _ => false,
                        };
                        if !should_create {
            ...
            // Only Active (single focused-output) mode exits when surfaces are gone.
            // AllScreens / TargetScreen / Background keep running so monitors can return.
            if window_state.units.is_empty() && window_state.is_active() {
                signal.stop();
                return true;
            }
        ...
        loop {
            ...
            // NOTE: we need to start the channel only after the dispatch run the first time
