    ...
    P::Message: 'static + TryInto<LayerShellCustomActionWithId, Error = P::Message>,
{
    ...
    fn handle_refresh_event(
        ...
        // End the unit borrow before mutating bindings / presenting.
        let layer_shell_id = layer_shell_window.id();
        // Persist binding so Closed can resolve iced id even without aliases.
        ev.set_unit_binding(layer_shell_id, iced_id);
        ...
        match compositor.present(
            ...
            Err(error) => match error {
                compositor::SurfaceError::OutOfMemory => {
                    // Monitor unplug / GPU reset often reports OutOfMemory. Drop
                    // the window instead of aborting the whole bar process.
                    tracing::error!(
                        "surface lost ({error:?}); dropping window {:?}",
                        layer_shell_id
                    );
                    ev.request_close(layer_shell_id);
                    self.handle_closed_event(ev, Some(layer_shell_id));
                }
                _ => {
    ...
    fn handle_closed_event(
        ...
        // Prefer the alias map: AllScreens bars often have no unit binding, and
        // units may already be gone when Closed is delivered after output destroy.
        let iced_id = self
            .window_manager
            .iced_id_for_layer(layer_id)
            .or_else(|| {
                ev.get_unit_with_id(layer_id)
