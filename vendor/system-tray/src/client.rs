            ...
            spawn(async move {
                while let Some(item) = stream.next().await {
                    ...
                    if let Ok(address) = address {
                        ...
                        .await
                        {
                            // Don't abort the registration stream — one bad SNI
                            // client (bad IconPixmap/ToolTip variants) must not
                            // take down the entire tray host.
                            error!("{err}");
                        }
                    }
                }
        ...
        macro_rules! get_property {
            ($name:expr) => {
                    ...
                    .await
                {
                    Ok(v) => Ok(Some(v)),
                    Err(e) => match e {
                        // Optional SNI properties (IconPixmap, etc.) are often absent.
                        zbus::fdo::Error::InvalidArgs(_) => {
                            debug!("optional property unavailable: {e}");
                            Ok(None)
                        }
        ...
        let property = match member.as_str() {
            ...
            "NewIcon" => {
                ...
                let icon_pixmap = get_property!("IconPixmap")
                    .unwrap_or_else(|e| {
                        debug!("IconPixmap unavailable: {e:?}");
                        None
                    })
                    .as_deref()
