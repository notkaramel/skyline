...
impl TryFrom<DBusProps> for StatusNotifierItem {
    ...
    fn try_from(props: DBusProps) -> Result<Self> {
        if let Some(id) = props.get_string("Id") {
            ...
            Ok(Self {
                id,
                title: props.get_string("Title").and_then(Result::ok),
                status: props.get_status().unwrap_or_default(),
                window_id: props
                    .get::<i32>("WindowId")
                    .and_then(Result::ok)
                    .copied()
                    .unwrap_or_default() as u32,
                icon_theme_path: props.get_string("IconThemePath").and_then(Result::ok),
                icon_name: props.get_string("IconName").and_then(Result::ok),
                // Pixmaps/tooltips often use quirky variants; never fail the whole item.
                icon_pixmap: props.get_icon_pixmap("IconPixmap").and_then(Result::ok),
                overlay_icon_name: props.get_string("OverlayIconName").and_then(Result::ok),
                overlay_icon_pixmap: props
                    .get_icon_pixmap("OverlayIconPixmap")
                    .and_then(Result::ok),
                attention_icon_name: props.get_string("AttentionIconName").and_then(Result::ok),
                attention_icon_pixmap: props
                    .get_icon_pixmap("AttentionIconPixmap")
                    .and_then(Result::ok),
                attention_movie_name: props.get_string("AttentionMovieName").and_then(Result::ok),
                tool_tip: props.get_tooltip().and_then(Result::ok),
                item_is_menu: props
                    .get("ItemIsMenu")
                    .and_then(Result::ok)
                    .copied()
                    .unwrap_or_default(),
                category: props.get_category().unwrap_or_default(),
                menu: props.get_object_path("Menu").and_then(Result::ok),
            })
        } else {
